...
export function parsePrototype(memitem, $) {
...
export function determineModule($, filename) {
    ...
    if (filename.includes('PlaybackDevice') || filename.includes('Audio'))
        return 'Audio';
    if (filename.includes('Glsl') || filename.includes('Shape') || filename.includes('Sprite') || filename.includes('Texture'))
        return 'Graphics';
    if (filename.includes('Dns') || filename.includes('Ftp') || filename.includes('Http') || filename.includes('Socket') || filename.includes('Packet') || filename.includes('IpAddress'))
        return 'Network';
    if (filename.includes('Window') || filename.includes('Event') || filename.includes('Keyboard') || filename.includes('Mouse') || filename.includes('Joystick') || filename.includes('Sensor') || filename.includes('Touch') || filename.includes('Clipboard') || filename.includes('Cursor') || filename.includes('VideoMode') || filename.includes('Vulkan') || filename.includes('Style'))
        return 'Window';
    return 'System';
}
...
export function buildIndex() {
    ...
    for (const { file, mod } of groupFiles) {
        ...
        $('.memitem').each((_, el) => {
            ...
            if (!title || title.includes('anonymous enum'))
                return;
    ...
    for (const file of classFiles) {
        ...
        let type = file.startsWith('struct') ? 'struct' : 'class';
        if (rawTitle.includes('Struct Reference'))
            type = 'struct';
        else if (rawTitle.includes('Class Reference'))
            type = 'class';
        let symbolName = rawTitle
            .replace(/\s*(Class|Struct|Reference|module|Graphics|Window|Audio|Network|System).*$/gi, '')
            .trim();
        if (!symbolName.startsWith('sf::') && !symbolName.startsWith('sf')) {
            const navItems = $('.navpath li').map((_, el) => $(el).text().trim()).get();
            if (navItems.length >= 2) {
                symbolName = navItems.join('::');
            }
            else {
    ...
    for (const file of nsFiles) {
        ...
        $('.memitem').each((_, el) => {
            ...
            if (proto.startsWith('enum'))
                ...
                kind = 'typedef';
            else if (proto.startsWith('const') && !proto.includes('('))
                kind = 'constant';
            members.push({
                name: title,
                signature: proto,
